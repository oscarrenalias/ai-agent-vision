"""
SQLite data storage implementation for the AI Agent Vision application.
This module provides a SQLite implementation of the DataStore interface.
"""
    ...
    def get_receipt_by_id(self, receipt_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT id, receipt_data, created_at, updated_at FROM receipts WHERE id = ?",
                    (receipt_id,),
                )
                row = cursor.fetchone()
